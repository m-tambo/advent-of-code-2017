...
let sample = `1919  2959  82  507 3219  239 3494  1440  3107  259 3544  683 207 562 276 2963
...
const getMax = (arr) => {
  const arr2 = arr.slice().sort((a, b) => a - b)
  return Number(arr2[arr2.length - 1])
}

const decode_utf8 = string => encodeURIComponent(string)

// const decoded = decode_utf8(sample)

const breakUpRows = string => string.split('%0A')
...
const checksum = (array) => {
  ...
  array.forEach((row) => {
    ...
    console.log( getMax(row), getMin(row) )
  })
  return sum
}

console.log(checksum(separateRowElements(breakUpRows(decode_utf8(sample)))))
// answer: 53978
